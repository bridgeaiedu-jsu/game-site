#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""run_mutations_four.py — verify_four.js 의 검출력 검산.

★이 파일은 run_mutations_push.py 를 ★옮겨 온 것이다(2026-09-07 · T0908-four).
골격(양방향 차집합·INDET 생산자·면제 사유 강제·지문 세 값)은 ★그대로 두었다 —
push 에서 여러 라운드에 걸쳐 단단해진 규율이라 새로 적으면 그 단단함이 사라진다.
바꾼 것은 ★가리키는 대상뿐이다.

무엇을 하나: `tools/verify_four.js --list-mutations` 로 뮤테이션 ★분모를 기계로 뽑고,
...
★주입 실패는 통과가 아니다. verify_four.js 는 셋을 가른다:
    rc=0  지목한 검사가 잡았다
...
VERIFY = os.path.join(ROOT, 'tools', 'verify_four.js')
LOCKED = os.path.join(ROOT, 'tools', 'four_locked_contracts.json')
...
def expectations():
    ...
    """
    path_ = os.path.join(ROOT, 'tools', 'four_mutation_expectations.json')
    try:
...
def main():
    ...
    ap.add_argument('--html', default=os.path.join(ROOT, 'four', 'index.html'))
    ...
    print(fingerprint_line('기대표 정본 지문', os.path.join(ROOT, 'tools', 'four_mutation_expectations.json'),
                           fingerprint(os.path.join(ROOT, 'tools', 'four_mutation_expectations.json'))))
