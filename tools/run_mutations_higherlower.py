...
# 대상에서 뮤테이션이 사라지면 목록도 함께 줄어 '32→31' 이 초록으로 지나간다.
...
MUTS = [
    ...
    ('bar-uneven-runtime', '막대 판에서는 무대가 좌우 균등 격자다(두 칸의 자가 같다)'),
    ('bar-even-css-widened', '값 순서와 눈에 보이는 크기 순서가 전 구간에서 일치한다'),
    ('dots-cur-bigger', '지금 칸이 값 그림의 치수를 다시 정하지 않는다(글자인 숫자만 예외)'),
    ('touch-target-shrunk', '답 칸은 360px 에서 짧은 변이 50px 하한을 지킨다'),
    ('answers-gap-widened', '이 셈이 실브라우저 실측(답 칸 폭)을 재현한다'),
]

EXPECTED_COUNT = 32          # ★tools/README.md 의 '사본 N종' 과 같은 수여야 한다(아래에서 대조한다)
...
def main():
    ...
    for qname, qwhy in QUIET:
        r = mutation_quiet_stage.quiet_control(
            ROOT, os.path.dirname(os.path.abspath(__file__)), 'higherlower', HTML,
