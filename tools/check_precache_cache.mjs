 ...
 *   · ★**base 가 head 의 조상이 아니면 아무것도 볼 수 없다** — 그래서 rc=2 로 멈춘다.
 *     기전: 그때 base 쪽의 CACHE identity 는 **head 의 first-parent 계보에 없다.** 이 도구는 계보에
 *     있는 것만 읽으므로 그 값을 ★볼 수조차 없다. 그래서 'A 가 v38 로 배포돼 있는데 B 가 v37 을 싣는다'
 *     같은 경우를 판별할 근거가 아예 없다 — B 는 공통 조상 이후로는 제대로 올렸으므로 어떤 계산을 해도
 *     초록이 나오지만, v38 을 가진 사용자에게는 새 버킷이 안 생긴다(이 도구가 축5 로 닫은 바로 그 병이다).
 *     ★`--base A --head B` 의 뜻은 둘이다: ①B 가 공통 조상 이후 무엇을 더했나 ②A 가 배포된 상태에서
 *     B 를 내보내도 안전한가. **이 게이트의 계약은 ②다**(출고되는 나무에 관한 것이므로).
 *     공통 조상을 시작점으로 삼으면 ①에 답하게 되는데, 그것은 ★물어보지 않은 질문에 조용히 답하는 것이다.
 *     둘이 우연히 같은 답을 내는 fixture 도 있어서 더 위험하다. 근거를 못 가진 질문에는 판정 불가가
 *     정직한 답이다 — 설계 원칙 3 과 같은 태도이고, 모르는 플래그를 rc=2 로 거부하는 것과도 일관된다.
 *     (정상 출고에서는 base 가 늘 origin/main 이고 그것은 항상 조상이라 이 rc=2 는 뜨지 않는다.)
 *     ※ 이 결정은 2026-09-03 에 세 번 뒤집힌 끝에 확정됐다. 되밟지 마라 — 되밟게 만든 것이
 *       '왜 볼 수 없는지' 가 안 적혀 있었다는 점이다.
 *
 * 사용법:
