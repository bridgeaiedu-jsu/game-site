...
RED = [
    ...
    # ── ② 링크 이어두기 — ★거부해야 할 것을 통과시키는 쪽이 위험하다(남이 만든 URL 이다)
    ('m-link-range', '불법 수열은 거부한다(판을 그리지 않는다)'),
    ('m-link-dup', '불법 수열은 거부한다(판을 그리지 않는다)'),
    ('m-link-afterend', '불법 수열은 거부한다(판을 그리지 않는다)'),
    ('m-link-b64', '불법 수열은 거부한다(판을 그리지 않는다)'),
