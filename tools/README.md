...
### 그 정지에 딱 하나 있는 예외 — **산문**만 좁게 연다 (2026-09-01)
사람에게 보여 줄 문안(`const msg = "localStorage 에 저장합니다"`)까지 멈추면 소음이 는다.
그래서 **멤버 이름이 될 수 없음이 구조적으로 성립하는** 자리만 연다 — 이름 양옆이 공백이나
비ASCII(한글 등)인 경우다. 그 글자들로는 자바스크립트 접근·호출 모양을 만들 수 없다.

경계를 '식별자가 될 수 없는 문자' 로 넓게 잡으면 **안 된다**. 문장부호가 함께 들어와
`"localStorage.setItem(…)"` 같은 간접 `eval` payload 가 이름 뒤 `.` 때문에 '더 긴 산문' 으로
읽혀 함께 열린다 — 실측으로 확인하고 기각한 안이다. 그래서 다음 셋은 예외에서 **제외**된다:

| 열지 않는 자리 | 왜 |
|---|---|
| 정확 일치 단독 문자열 `'localStorage'` | 그대로 멤버 이름이 될 수 있다(`Reflect.get(window, K)`) |
| `.` `+` `[` 에 물린 문자열 | `"localStorage 저장".slice(0, 12)` 는 잘라 내면 이름이다 |
| 치환이 든 템플릿 `` `localStorage ${n}건` `` | 글자 그대로가 아니라 **조립되는** 문자열이다 |

자기시험이 이 경계를 짝 케이스 넷으로 못박는다(정확 일치 단독 · 산문을 잘라 이름으로 ·
`window["localStorage"]` 첨자 · 간접 eval payload). 그리고 변이체 둘이 **완화 자체**를 잡는다 —
경계에 문장부호를 허용하면 간접 eval payload 가, 문자열 연산 가드를 지우면 잘라 쓰는 경로가
각각 rc=0 으로 되돌아간다. 다음에 같은 완화가 제안돼도 이 둘이 막는다.
