...
node tools/check_functions.mjs . --selftest               # 내장 검출력 자기시험(20항목)
...
뜻이므로 자기시험이 FAIL 한다(공허한 통과 차단). 지금 걸려 있는 방어는 셋이다:
판정 불가→rc=2 · 데이터 모듈 default-only · 런타임 모듈 manifest 대조.
