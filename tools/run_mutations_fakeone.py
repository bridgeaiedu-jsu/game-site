#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""verify_fakeone.js 의 검출력 검산 — worker(238) · 2026-09-04 · 티켓 T0904-fake-one

`verify_fakeone.js` 가 **고의 결함을 정말로 잡는지** 를 뮤테이션 전수 주입으로 검산한다.
...
VERIFIER = os.path.join(ROOT, 'tools', 'verify_fakeone.js')
...
HTML = arg_of('--html', os.path.join(ROOT, 'fake-one', 'index.html'))
...
# 대상에서 뮤테이션이 사라지면 목록도 함께 줄어 '26→25' 가 초록으로 지나간다.
# 아래는 이 러너가 **독립 리터럴로** 쥔 (뮤테이션 이름, 이 결함을 잡아야 하는 검사) 쌍이고,
# 대상이 내놓는 --list-mutations 는 근거가 아니라 ★대조 상대다.
MUTS = [
    ('frame-accumulate', '그린 시계가 매 프레임 흐른 시간과 같다(프레임 간격 불규칙)'),
    ('judge-from-drawn', '총 기록은 마지막으로 그린 시계가 아니라 누른 순간의 도장에서 나온다'),
    ('rng-on-play', '플레이 행동은 난수를 한 번도 소비하지 않는다(오늘의 도전)'),
    ('seed-drift', '같은 씨앗은 같은 판을 준다(8라운드 전체)'),
    ('defer-commit', '타이머를 한 번도 돌리지 않아도 오답 벌점이 즉시 확정된다'),
    ('penalty-not-counted', '오답 1회는 총 기록에 정확히 2초를 더한다'),
    ('wrong-advances-round', '오답을 눌러도 라운드는 넘어가지 않는다'),
    ('two-fakes', '격자에서 나머지와 다른 칸은 정확히 하나다(그려진 그림으로 셈)'),
    ('axis-mix', '한 라운드는 차이축을 하나만 쓴다(가짜는 그 축 하나만 어긋난다)'),
    ('diff-not-shrinking', '라운드가 오를수록 차이가 작아진다(amt 가 라운드마다 줄어든다)'),
    ('grid-not-growing', '라운드가 오를수록 격자가 커진다'),
    ('axis-balance-broken', '여덟 라운드에 네 축이 두 번씩 쓰인다'),
    ('en-prose-missing', '마크업의 data-i18n 키가 ko·en 두 표에 모두 있다'),
    ('best-worse-wins', '자유 모드 최고 기록은 더 짧은 시간일 때만 바뀐다'),
    ...
    ('lang-rebuilds-board', '진행 중 언어를 바꿔도 눌린 자리 표시가 사라지지 않는다'),
    ('fig-out-of-gamut', '색상축의 모든 밝기 단계가 sRGB 안에 있다(잘리면 렌더된 차이가 선언보다 줄어든다)'),
    ('fig-low-contrast', '색상축의 모든 밝기 단계가 칸 바탕 대비 3:1 을 넘는다'),
    # ── master(236) 독립 검증이 뚫은 사각 2건을 막고 세운 짝 ──────────────
    ('daily-seed-uses-clock', '같은 씨앗은 시각이 달라져도 같은 판을 준다(가짜 시계로 크게 벌려 확인)'),
    ('grid-cap-raised', '가장 큰 격자도 360px 에서 칸 한 변 50px 을 지킨다'),
    ('board-gap-widened', '이 셈이 실브라우저 실측 두 점(5x5 55.84px · 6x6 45.53px)을 재현한다'),
]

EXPECTED_COUNT = 26          # ★tools/README.md 의 '사본 N종' 과 같은 수여야 한다(아래에서 대조한다)
README_COUNT_RE = re.compile(r'가짜 하나 의 계약을 하나씩 깨뜨린 사본 (\d+)종')
...
def main():
    ...
    for qname, qwhy in QUIET:
        r = mutation_quiet_stage.quiet_control(
            ROOT, os.path.dirname(os.path.abspath(__file__)), 'fakeone', HTML,
            VERIFIER, SUMMARY_RE, MUTS[0][0])
        print('')
