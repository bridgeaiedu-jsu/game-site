...
MUTS = [
    ...
    ('m-clock-via-now', 'higher-lower/index.html',
     'const k = new Date((d ? d.getTime() : new Date().getTime()) + 9*3600000);',
     'const k = new Date((d ? d.getTime() : Date.now()) + 9*3600000);',
     'migrate-dynamic', '시계를 Date.now() 로 읽게 되돌린다 — 생성자만 덮는 가짜 시계를 우회해 관측이 사라진다(값은 같고 검사 가능성만 없어지는 부류 · 2026-09-09 실측 회귀)'),
