...
8. ★**병합한 나무에서 게이트를 다시 돌린 뒤 push 한다.**
   ...
   - [ ] (「함께 탈출」을 손댄 라운드에서만) `node tools/verify_talchul.js` → **rc=0 일 때만** 통과다.
         제품보다 먼저 쓰인 검증기다 — 제품이 없으면 rc=2(판정 불가)이고 통과로 세지 않는다.
         재는 계약: 같은 KST 날이면 같은 판 · ★두 말의 자리를 함께 훑어 해답이 있다(독립 너비우선
         탐색) · 제품이 말하는 최소 총 이동 수가 그 탐색과 같다 · ★문을 닫아 두면 안 풀린다(문이
         장식이 아니다) · 최소 수순에서 두 말이 모두 움직인다 · ★영구적으로 갇히는 자리가 0 이다 ·
         문이 닫힐 칸에 선 말은 쫓겨나지 않고 나올 수 있다 ·
         ★한 말만 출구에 서면 아직 끝이 아니다 · 차례 넘기기는 이동 수에 안 든다 · 되돌리기는 한 수 ·
         벽·대각·닫힌 문·동료 칸 입력은 무효이고 벌점이 없다.
         ★보고서에서 의도적으로 벗어난 자리: 보고서의 "출구에 든 말은 이동을 마친다"를 쓰면 먼저
         나간 쪽이 동료를 영구히 가두는 판이 생긴다. 그래서 출구를 드나들 수 있는 칸으로 두고
         승리 조건만 ★동시에로 바꿨다 — 그 대가로 「갇히는 자리 0」을 계약으로 요구한다.
         ★처음 쓴 「한쪽 말만 움직여 풀리는가」는 ★공허했다 — 승리 조건이 두 말 모두 자기 출구에
         서는 것이라 한 말을 못 박으면 어떤 판이든 자동으로 안 풀린다. 그래서 재는 대상을 ★문의
         필요성으로 바꿨다(자세한 사유는 검증기 파일 머리말). 그 계기가 실제로 반응하는지는
         ★양성 대조(일부러 옆길을 둔 작은 판)로 함께 증명한다.
         ★탐지력은 `node tools/verify_talchul.js --selftest` 로 잰다(뮤테이션 10종 · 어긋남 0 이 합격선).
   - [ ] (「규칙 찾기」를 손댄 라운드에서만) `node tools/verify_gyuchik.js` → **rc=0 일 때만** 통과다.
         제품보다 먼저 쓰인 검증기다 — 제품이 없으면 rc=2(판정 불가)이고 통과로 세지 않는다.
         재는 계약: 같은 KST 날이면 같은 5문제와 ★같은 보기 차례 · 규칙군은 선언한 넷 안 ·
         보기 넷이 서로 다르고 정답이 하나 · 제품의 정답이 ★내가 따로 이어 본 다음 칸과 같다 ·
         ★모호함 0(두 겹: 문제마다 + ★규칙군끼리 전수 대조 42만 가지) · 이어지는 7일 겹침 0 ·
         난수·시계 없음 · 점수는 맞힌 개수(감점·시간 가산 없음) · 힌트는 ★규칙군만 알려 준다 ·
         틀려도 안 끝나고 제출은 한 번 · 해설이 정답 칸을 그대로 적지 않는다.
         ★뮤테이션이 ★무력한 계약이 둘 있다(모호함 거름망 · 보기 겹침 거름망) — 구조적으로
         그 일이 안 일어나기 때문이다. 그 둘의 탐지력은 ★양성 대조 세 개로 대신 증명한다
         (제품 선언한 판 크기 대조 · 일부러 겹치는 다섯째 규칙 · 일부러 겹친 보기).
         ★탐지력은 `node tools/verify_gyuchik.js --selftest` 로 잰다(뮤테이션 7종 · 어긋남 0 이 합격선).
   - [ ] (「단어 다리」를 손댄 라운드에서만) `node tools/verify_dari.js` → **rc=0 일 때만** 통과다.
         제품보다 먼저 쓰인 검증기다 — 제품이 없으면 rc=2(판정 불가)이고 통과로 세지 않는다.
         재는 계약: 같은 KST 날이면 같은 시작·목표 · 그 문제가 ★내 독립 너비우선 탐색으로 풀린다 ·
         제품이 말하는 최단 변환 횟수가 ★그 탐색의 최단과 같다 · 최단이 규격 범위(2~5단계) 안 ·
         이어지는 7일 겹침 0 · 난수·시계 없음 · ★두 음절이 다 바뀐 말·사전에 없는 말·되풀이하는 말은
         막고 이유를 말하며 ★시도를 깎지 않는다 · 최단이 아닌 올바른 길도 완주로 인정된다 ·
         힌트는 단계적이다(1단계는 ★바꿀 자리만, 2단계라야 말) · 사전은 두 글자 한글뿐·중복 0 이고
         ★끝말잇기 사전의 두 글자 묶음과 바이트 그대로 같다(11,484개 · 쉬움 1,068개는 그 부분집합).
         ★못 보는 것(정직 고지): 그 말이 정말 표준어인지·뜻이 자연스러운지는 재지 못한다 —
         풀이는 제품을 믿지 않고 따로 하지만 ★사전 자체는 공유한다.
         ★탐지력은 `node tools/verify_dari.js --selftest` 로 잰다(뮤테이션 9종 · 어긋남 0 이 합격선).
   - [ ] (「라이어 게임」을 손댄 라운드에서만) `node tools/verify_liar.js` → **rc=0 일 때만** 통과다.
         제품보다 먼저 쓰인 검증기다 — 제품이 없으면 rc=2(판정 불가)이고 통과로 세지 않는다.
         재는 계약: 제시어 표(주제 8개·주제마다 40개 이상·한글만·★주제 안·사이 겹침 0) · 판 짜기는 순수하다
         (같은 씨앗=같은 판 · Math.random·시각 읽기 0) · ★라이어는 정확히 1명이고 8명 판 400번에 모든 자리가 라이어가 된다 ·
         ★기본 모드 라이어 카드에는 제시어가 ★없다(숨김이 아니라 부재) · 바보 모드 라이어는 같은 주제의 다른 낱말을 받고
         화면에 「라이어」가 없다 · 역전 보기 8개(겹침 0·제시어 포함·같은 주제) · 최근 30개 낱말은 다시 뽑지 않는다 ·
         ★누르기 전엔 비어 있고 손을 떼거나 화면이 숨으면 글자를 ★지운다 · 한 번도 보지 않고는 확인을 못 누른다 ·
         지목(시민→라이어 도망 · 라이어→역전 기회 → 맞히면 역전·틀리면 시민 승) · 한 판 동안 네트워크 0 · ko/en 문구 키 대칭.
         ★못 보는 것(정직 고지): 낱말이 정말 모두가 아는 말인지·상표·실존 인물이 아닌지는 사람이 검수한다. 360px 넘침은 실브라우저로 잰다.
         ★탐지력은 `node tools/verify_liar.js --selftest` 로 잰다(뮤테이션 10종 · 어긋남 0 이 합격선).
