...
8. ★**병합한 나무에서 게이트를 다시 돌린 뒤 push 한다.**
   ...
   - [ ] (「빠른 셈」 검증기를 건드린 라운드에서만 · 느리다) `python3 tools/audit_term_denominator.py .` →
         ★단언이 둘 이상인 검사의 **각 항이 실제로 분모를 갖는지** 잰다 — 항 하나만 지운 사본으로
         그 검사를 겨냥한 뮤테이션을 다시 돌려, 지워도 잡히면 그 항은 **아무 변이도 기대지 않는다**.
         ★이 수치를 보고에 쓰려면 **이 도구가 찍은 줄**을 그대로 옮겨라(손계산 금지 · 2026-09-08 R5 F13).
         rc 는 판정이 아니라 실행 성공 여부다 — 결과는 표로 읽는다.
   - [ ] `node tools/check_theme_contrast.mjs .` → **rc=0 일 때만** 통과다. 두 테마(라이트·다크)에서
         ★렌더된 색으로 글자 대비비를 잰다(보통 4.5:1 · 큰 글자 3:1 · `Emulation.setEmulatedMedia` 로
         다크를 강제하고 `getComputedStyle` 로 읽는다 · 요소를 고르지 않고 ★자기 글자를 가진 보이는
         요소 전수). `--selftest` 로 검출력을 잰다(판정은 ★기준선 대비 증가·감소로 한다).
         ★왜 always 인가(2026-09-08 R6 B5): 대비는 **제품이 안 바뀌어도 토큰·테마가 바뀌면 깨지고**,
         "표면 위에 새 글자가 놓이는 변경" 은 조건부 트리거로 원리적으로 못 덮는다. 실제로 이 게이트를
         세우자마자 결과 화면에서 위반 4건이 나왔고(맞힘·틀림·정확도 라벨 4.19:1 · 광고 자리 안내 4.25:1),
         ★그 자리들만 제품이 이미 4.5 이상이라 선언한 토큰(`--muted-2`)으로 바꿔 0 으로 만들었다.
         ★면제 목록은 두지 않는다 — 고치기 전에 면제 장치를 두면 면제가 수리를 대신한다(master 판정).
   ...
   - [ ] `node tools/check_daily_boundary.mjs .` → **rc=0 일 때만** 통과다. 하루 경계가
         **KST(UTC+9) 하나인지**를 잰다(오너 결정 2026-09-09 · 공개 문구 '전 세계 같은 문제'와의 정합).
         정적 전수(모든 일일 게임의 `dayKey`·회차 파생·`prevDayKey` UTC 산술·저장의 시작 시점 못박기·
         스트릭 구 로컬 키 1회 이관·완료 잠금에 관용 없음)와 **실행 판정**을 함께 낸다.
         실행 판정은 **TZ 를 강제한 자식 프로세스**에서 돈다 — 검사 기계가 마침 KST 면 로컬과 KST 가
         같아져 어떤 코드든 통과하기 때문이다(공허 차단). 이행 판정은 관용이 실제로 필요한
         **UTC+9 초과 시간대**에서 돌리고, 로컬 날짜와 KST 날짜가 갈렸다는 **전제**를 먼저 단언한다 —
         갈리지 않으면 통과가 아니라 **판정 불가(rc=2)** 다.
   - [ ] `node tools/check_ad_placeholder.mjs .` → **rc=0 일 때만** 통과다. 광고
         **자리표시자**(`.ad-slot` 중 `.live` 가 아닌 자리)의 글자가 **화면에 그려지지 않는지**를 잰다.
         애드센스 심사 동안 빈 '광고 자리' 상자를 감추는 스위치는 `css/ad-slots.css` **한 곳**이고,
         이 게이트는 그 스위치가 **실제 렌더 결과에 닿는지**를 본다. 소스 grep 은 마크업에 그 글자가
         있다까지만 증명하므로 쓰지 않는다. **진짜 브라우저**를 띄우되 **HTTP 로** 띄운다 —
         `file://` 에서는 절대경로 `/css/ad-slots.css` 가 풀리지 않아 스위치가 로드조차 안 되고
         그 초록은 거짓이다. 대상은 하드코딩 목록이 아니라 **트리에서 파생**하고(`.ad-slot` 보유 페이지
         전수) **ko·en 두 언어**를 각각 센다. 함께 **검출력 자기시험**을 돌려 숨김 규칙을 무르게 하면
         이 검사기가 실제로 붉어지는지 보이고, 그것이 서지 않으면 통과가 아니라 **판정 불가(rc=2)** 다.
   - [ ] `python3 tools/check_article_uniqueness.py .` → **rc=0 일 때만** 통과다. 게임 페이지에
         실린 **읽을 글**(`.read-ko`/`.read-en`)이 서로 **복제가 아닌지**를 잰다. 같은 문장을 게임
         이름만 바꿔 돌려 쓰면 중복 콘텐츠가 되어 **더하는 쪽이 손해**이기 때문이다. 두 축을 함께
         본다 — 쌍마다 **완전히 같은 문장의 교집합 0**, 그리고 **문자 5-그램 자카드 유사도**가 임계
         미만. 말끝만 바꾸는 우회는 첫 축을 빠져나가고 두 번째 축에서 걸린다. 글 길이(ko 800~1,500자)와
         **ko·en 두 벌 존재**도 같은 자리에서 센다. 대상은 하드코딩 목록이 아니라 **트리에서 파생**한다.
         매 실행 **복제본 자기시험**을 돌려 이름만 바꾼 복제가 실제로 임계를 넘는지 보이고, 그 폭이
         벌어지지 않으면 통과가 아니라 **판정 불가(rc=2)** 다.
   - [ ] `python3 tools/run_mutations_kst.py` → **rc=0 일 때만** 통과다. 위 게이트의 **검출력**을
         뮤테이션으로 보인다: KST 시프트 되돌리기·회차만 로컬로·prevDayKey 로컬 경유·저장 시점 호출·
         스트릭 이행 제거를 **게임군 대표**에 넣고 **지목한 검사**가 붉어지는지 본다(무임승차 불인정).
   ...
   - [ ] **그 라운드에서 손댄 게임의 검증기**를 ★커밋본 바이트로 돌린다. 「빠른 셈」이면
         `node tools/verify_quickmath.js --from-commit <이번 커밋 해시>` → **rc=0 일 때만** 통과다
         (`rc=1` 미달 · `rc=2` **판정 불가** · `rc=3` 검사가 공허하다 — 셋 다 배포하지 않는다).
         ★왜 커밋본인가: 워킹트리로 재면 ★체크아웃 줄끝이 뮤테이션 앵커를 갈라 놓는다. 이 저장소는
         blob 이 LF 이고 Windows 체크아웃은 CRLF 라, `\r\n` 이 박힌 앵커는 **개발자 트리에서만** 맞았다
         (2026-09-07 R2 실측: 커밋본에서 **3건 주입 실패** — 그중 하나가 지목한 포커스 검사는 그
         환경에서 **한 번도 검증된 적이 없었다**). 검사기는 자기가 잰 바이트(경로·sha256·줄끝 수)를
         ★출력 첫 줄에 적는다 — 증거에는 **그 줄을 그대로** 옮겨라. "내 트리에서 통과" 는 증거가 아니다.
   - [ ] `python3 tools/run_mutations_quickmath.py --from-commit <이번 커밋 해시>` → **rc=0 일 때만** 통과다.
         검증기가 초록이라는 것은 "검사가 살아 있다" 는 뜻이 아니다 — 잡아야 할 것을 주입했을 때
         **그 검사가** 붉어지는지를 봐야 초록에 뜻이 생긴다. 이 러너는 결과를 ★네 통으로 가르고
         (겨냥 검출 · 대조군 조용 · 공허 · 거짓 실패 · 판정 불가) **합산하지 않는다**. 분모는 손으로
   ...
   - [ ] (「한붓 배달」을 손댄 라운드에서만) `node tools/verify_hanbut.js` → **rc=0 일 때만** 통과다.
         제품보다 먼저 쓰인 검증기다 — 제품이 없으면 rc=2(판정 불가)이고 통과로 세지 않는다.
         재는 계약: 같은 KST 날이면 벽시계를 23시간 40분 옮겨도 같은 판 · 날짜가 다르면 판도 다르다 ·
         그날의 판은 반드시 풀린다(★이 파일의 ★독립 솔버로 다시 푼다) · 제품이 말하는 최단 이동 수가
         그 독립 계산과 같다 · 시작·출구·배달은 서로 다른 칸이고 벽이 아니다 · 판을 짜는 데 난수를
         안 쓰고 그 함수에 시각 읽기가 없다 · 추가 = 내 이동 − 최단(음수 불가) · 되돌리기는 한 칸 ·
         벽·대각·이미 지난 칸 입력은 무효이고 이동 수가 늘지 않는다.
         ★탐지력은 `node tools/verify_hanbut.js --selftest` 로 잰다(뮤테이션 5종 · 어긋남 0 이 합격선).
   - [ ] (「돌려 맞춰」를 손댄 라운드에서만) `node tools/verify_dolryeo.js` → **rc=0 일 때만** 통과다.
         제품보다 먼저 쓰인 검증기다 — 제품이 없으면 rc=2(판정 불가)이고 통과로 세지 않는다.
         재는 계약: 같은 KST 날이면 벽시계를 옮겨도 같은 판(배치·처음 방향·잠금까지) · 그날의 판은
         반드시 맞출 수 있다(★전체 방향 전수 탐색) · 제품이 말하는 최소 회전 수가 그 전수 탐색과
         같다 · 판정이 ★정답 일치가 아니라 ★실제 연결 상태다(검증기가 ★답이 둘인 판을 따로 세워
         묻는다 — 제품이 낸 판은 길이 하나뿐이라 그 차이를 잴 수 없다) · 시작과 무관한 고립 고리는
         완성이 아니다 · 길 끝이 새면 완성이 아니다 · 직선의 180도는 같은 상태다 · 잠긴 조각은
         돌지 않고 벌점도 없다 · 되돌리기는 한 번 · 네 번 돌리면 제자리(판정 누락 없음).
         ★잠금 조각이 하나도 없는 판만 나오면 rc=2 다 — 빈 검사를 통과로 세지 않는다.
         ★탐지력은 `node tools/verify_dolryeo.js --selftest` 로 잰다(뮤테이션 9종 · 어긋남 0 이 합격선).
